...
def get_identity_prompt() -> str:
    """获取人设提示词"""
    # 1. 尝试从配置文件加载
    identity_file = Path.home() / ".adk-claw" / "IDENTITY.md"
...
def get_default_identity() -> str:
    """获取默认人设"""
    agent_name = config.config.get("identity", {}).get("name", "ADK Claw")
...
def load_identity_from_file(file_path: str):
    ...
    # 保存到 ~/.adk-claw/IDENTITY.md
    target = Path.home() / ".adk-claw" / "IDENTITY.md"
